...
| 36 | `20260725250000_wire_storage_object_refs_into_asset_lifecycle.sql` | **PR #12 fix.** Wire `storage_object_refs` registry into the asset lifecycle: draft create, draft replace, publish finalize (catalog + certificate), cleanup completion. Ensures every private/public object has an active ref, old refs are marked superseded, and cleanup completion marks refs as deleted. Prevents orphaned private objects. | pending | pending | pending | — | — | — | Drop the helper RPCs and restore prior RPC bodies (would leave storage_object_refs unused). |
