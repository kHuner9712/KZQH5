...
| 49 | `20260801000000_distributed_rate_limit_rpc.sql` | **KZQ-P1-011-b.** Add `rate_limit_counters` table (bucket/key/window_start PK, RLS enabled, revoke from public/anon/authenticated) + `rate_limit_check(text, text, integer, integer)` atomic fixed-window increment RPC + `rate_limit_cleanup_expired(integer)` best-effort cleanup RPC. Both RPCs: SECURITY INVOKER, empty search_path, EXECUTE to service_role ONLY. Backs the PostgresRateLimiter driver (`RATE_LIMIT_DRIVER=postgres`) for multi-instance shared rate limiting. | pending | pending | pending | — | — | — | Drop the 2 functions + table. Application falls back to MemoryRateLimiter (fail-open per P1-011-a decision matrix). |
