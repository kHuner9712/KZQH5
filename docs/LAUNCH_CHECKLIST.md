...
- [ ] **`npm run check:release-readiness -- --mode=production` 在正式发布前通过**（exit 0；新增的 TRUSTED_PROXY_HEADER / RATE_LIMIT_FALLBACK_SECRET / OUTBOX_DISPATCH_SECRET / BUILD_MOCK_BACKEND / loopback Site URL / loopback Supabase URL / DEMO_MODE / service role 暴露 等 BLOCK 条件全部满足。CSP_ENFORCING 不是 BLOCK 条件：admin 路由无条件 Report-Only，公共路由在 CSP_ENFORCING=true 时切换为 Enforcing 并保留 'unsafe-inline' 以兼容 ISR）
