...
- Last updated: 2026-07-31 (KZQ-P0-001 completed)
...
**KZQ-P0-002** — Correct signed upload URL lifecycle model (DB expiry vs
Supabase capability TTL).

KZQ-P0-001 is now completed. KZQ-P0-002 is the next P0 atomic task (not a
workstream). Its scope is documentation + code clarification in
`lib/services/two-phase-upload.ts` and `docs/TWO_PHASE_UPLOAD_DESIGN.md`
distinguishing the DB `expires_at` (business authorization window) from the
Supabase `createSignedUploadUrl` capability URL TTL. It requires no remote
DB write and no migration unless a DB adjustment is deemed necessary.
