...
- Last updated: 2026-07-31 (KZQ-P0-005-b completed)
...
| KZQ-P0-005 | P0 | Epic A 两阶段上传 | Unify single-stage and two-stage storage saga (workstream — split into atomic sub-tasks) | in_progress | — | — | per sub-task | Workstream split into sub-tasks a-g below. Sub-tasks a (unified validation) and b (unified final path generation) completed. Remaining: c (unified audit-start), d (unified audit-complete), e (unified compensation), f (unified cleanup/reconciliation), g (unified storage object reference registration) |
...
**KZQ-P0-005-c** — Unify audit-start.

KZQ-P0-005-a (unified validation) and KZQ-P0-005-b (unified final path
generation) are completed. KZQ-P0-005-c audits whether the two upload
paths share the same audit-start logic (`recordStorageAuditStarted` /
`beginStorageAudit`). The single-stage path defines `recordStorageAuditStarted`
in `storage-upload.ts`; the two-stage path may or may not reuse it for its
finalize-time audit. Audit real code to confirm whether drift exists, then
unify or lock the contract with tests.
