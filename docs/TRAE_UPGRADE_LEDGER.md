...
|----|----------|------------|------|--------|--------|--------|------------|-------|
| KZQ-P0-001 | P0 | Epic A 两阶段上传 | Verify `complete_temp_upload_finalize` RPC business return value (`ok` field) | pending | — | — | `npx vitest run tests/unit/two-phase-upload.test.ts` | `lib/services/two-phase-upload.ts:462-479` destructures only transport error; never reads returned `data.ok`; `ok:false` returns success to caller |
| KZQ-P0-002 | P0 | Epic A 两阶段上传 | Correct signed upload URL lifecycle model (DB expiry vs Supabase capability TTL) | pending | — | — | `npx vitest run tests/unit/two-phase-upload.test.ts` | `lib/services/two-phase-upload.ts:206` `createSignedUploadUrl(path)` accepts no TTL; `docs/TWO_PHASE_UPLOAD_DESIGN.md:100` claims 5-min TTL that code cannot enforce |
...
**KZQ-P0-001** — Verify `complete_temp_upload_finalize` RPC business return
value.

It is the highest-priority pending atomic task (not a workstream). Its scope is
contained to `lib/services/two-phase-upload.ts` finalize handling plus unit
tests, and it requires no external decision, no remote DB write, and no
migration.
