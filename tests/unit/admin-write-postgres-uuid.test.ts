...
describe("admin PostgreSQL UUID validation", () => {
  ...
  it("accepts the nested product envelope emitted by the browser form", () => {
    ...
    if (result.ok) {
      ...
      expect(result.value.product.fire_rating).toBeNull();
      expect(result.value.product.eco_grade).toBeNull();
    }
  });
});
