  ...
  return (
    <div
      ...
      style={
        cover
          ? {
              backgroundImage: `linear-gradient(rgba(13,15,16,.42),rgba(13,15,16,.72)),url(${cover})`,
    ...
    >
      ...
      <div className="relative flex h-full flex-col justify-between">
        <div className="flex items-start justify-between gap-3">
          <span className="text-[10px] font-medium uppercase tracking-[0.24em] text-gold-light">
            KZQ
          </span>
        ...
        <div>
          <p className="max-w-[15rem] text-lg font-semibold leading-tight md:text-xl">
            {title}
          </p>
          <p className="mt-2 line-clamp-1 text-[10px] uppercase tracking-[0.12em] text-white/60">
            {subtitle}
          </p>
        </div>
      </div>
...
export function CatalogTopicGrid({
  items,
  locale,
}: {
  items: CatalogTopicGridItem[];
  locale: Locale;
}) {
    ...
    <>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((item) => {
          const description =
            locale === "zh"
              ? item.topic.descriptionCn
              : item.topic.descriptionEn;
          const cardBody = (
            <>
              ...
              <div className="p-4">
                ...
                <p className="mt-3 min-h-10 text-xs leading-5 text-ink-soft">
                  {description}
                </p>
                {item.asset?.published_at && (
                  <p className="mt-2 text-[10px] text-ink-mute">
                    {labels.date}: {item.asset.published_at}
                  </p>
                )}
