...
export async function AboutPageContent(locale: Locale) {
  ...
  if (isDemoMode()) company = mockCompany; else { const { data, error } = await createPublicSupabaseClient().from("company_profile").select("*").limit(1).maybeSingle(); if (error) throw new Error("PUBLIC_DATA_UNAVAILABLE", { cause: error }); company = (data as CompanyProfile | null) || null; }
  ...
  const organization = { "@context": "https://schema.org", "@type": "Organization", name: "KZQ", url: siteUrl(localePath(locale)), logo: company?.logo_url || undefined, description: localizedCompany.description || undefined, inLanguage: localeConfig[locale].htmlLang, address: localizedCompany.address ? { "@type": "PostalAddress", streetAddress: localizedCompany.address } : undefined, contactPoint: { "@type": "ContactPoint", telephone: company?.phone || undefined, email: company?.email || undefined, contactType: "sales" } };
