...
export async function ContactPageContent(locale: Locale, searchParams: ContactSearchParams) {
  ...
  return (
    <div className="animate-fade-in bg-canvas">
      ...
      <ResponsiveContainer className="py-8 md:py-12"><div className="md:grid md:grid-cols-5 md:gap-10 lg:gap-14"><div className="md:col-span-2"><SectionHeader title={methods?.title || copy.methods} size="large" /><div className="mt-4 space-y-2.5 md:mt-6 md:space-y-3">{company?.phone && <ContactCard icon="phone" label={copy.phone} value={company.phone} href={`tel:${company.phone.replace(/[^+\d]/g, "")}`} />}{company?.email && <ContactCard icon="email" label={copy.email} value={company.email} href={`mailto:${company.email}`} />}{company?.whatsapp && <ContactCard icon="whatsapp" label="WhatsApp" value={company.whatsapp} href={`https://wa.me/${company.whatsapp.replace(/[^\d]/g, "")}`} external />}{localizedCompany.address && <ContactCard icon="address" label={copy.address} value={localizedCompany.address} />}</div><ContactQuickActions phone={company?.phone} wechat={company?.wechat} locale={locale} />{company?.wechat_qr_url && <div className="mt-4 flex flex-col items-center rounded-2xl border border-ink-line bg-canvas-warm p-4 md:mt-6"><div className="flex items-center gap-1.5 text-[11px] text-ink-soft"><QrCode className="h-4 w-4 text-industrial" />{copy.wechat}</div><div className="mt-3 overflow-hidden rounded-lg bg-white p-1.5"><QRCodeImage src={company.wechat_qr_url} className="h-32 w-32 md:h-36 md:w-36" /></div><p className="mt-2 text-[11px] text-ink-mute">{copy.wechatSales}</p></div>}</div><div className="mt-8 md:col-span-3 md:mt-0"><SectionHeader title={formSection?.title || copy.form} subtitle={formSection?.subtitle || formSection?.body || copy.formSubtitle} size="large" /><div className="mt-4 md:mt-6"><InquiryForm defaultProduct={searchParams.product || ""} productContext={{ id: searchParams.product_id, slug: searchParams.product_slug, name: searchParams.product, pageUrl: searchParams.page_url }} locale={locale} /></div></div></div></ResponsiveContainer>
    </div>
  );
}
