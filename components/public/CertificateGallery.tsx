...
export function CertificateGallery({
  ...
  const show = (index: number) => {
    ...
    resetTransform();
    trackAnalyticsEvent({ event_name: "certificate_view", locale });
  };
  const close = () => {
    setActiveIndex(null);
    resetTransform();
    ...
    <>
      <div
        className={
          compact
            ...
            : "grid grid-cols-2 gap-2.5 md:grid-cols-3 md:gap-4 lg:grid-cols-4"
        }
      >
        {certificates.map((certificate, index) => {
          ...
          return (
            ...
            >
              <div
                className={
                  compact ? "relative aspect-[4/3]" : "relative aspect-[3/4]"
                }
              >
                <ProductImage
                ...
                />
                <span className="absolute left-2 top-2 rounded bg-black/60 px-2 py-1 text-[9px] text-white">
                  {copy.displayVersion}
                </span>
              </div>
              <div className="p-3">
                <h3
                  className={
                    compact
                      ? "line-clamp-2 text-xs font-medium text-white"
                  ...
                  <p
                    className={
                      compact
                        ...
                        : "mt-2 flex items-center gap-1 text-[10px] text-ink-mute"
                    }
                  >
      ...
      {active && activeContent && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={activeContent.name}
        >
          <div className="safe-top flex min-h-16 items-center gap-3 border-b border-white/10 px-3 text-white md:px-6">
          ...
          <div className="relative min-h-0 flex-1 overflow-hidden">
              ...
              {active.image_url ? (
                <div className="flex h-full w-full items-center justify-center p-6 md:p-12">
                  <Image
                    ...
                    draggable={false}
                    className="max-h-full w-auto max-w-full object-contain transition-transform duration-75"
                    style={{
            ...
            {certificates.length > 1 && (
              <>
                <button
                  ...
                  className="absolute left-2 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-white"
                ...
                <button
                  type="button"
                  onClick={() => show(activeIndex! + 1)}
                  className="absolute right-2 top-1/2 flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full bg-black/55 text-white"
            ...
            <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2">
              ...
              <button
                ...
                className="min-w-16 rounded-full bg-black/65 px-3 text-xs text-white"
              >
          ...
          <div className="safe-bottom border-t border-white/10 bg-page px-4 py-3 text-white">
            <p className="text-xs text-white/70">{activeContent.description}</p>
            {activeContent.applicableScope && (
              <p className="mt-1 text-xs text-gold-light">
                {locale === "zh" ? "适用范围" : "Applicable scope"}:{" "}
                {activeContent.applicableScope}
              </p>
            )}
            <p className="mt-1 text-xs text-white/50">{copy.fullDocs}</p>
          </div>
        </div>
      )}
    </>
