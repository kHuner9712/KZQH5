...
function LanguageSwitcherInner({ locale, className }: { locale: Locale; className?: string }) {
  ...
  return <Link href={href} prefetch={false} hrefLang={localeConfig[target].htmlLang} className={cn("inline-flex min-h-9 items-center text-xs font-medium", className)} aria-label={copy.header.switchLanguage}>{copy.header.languageShort}</Link>;
}
