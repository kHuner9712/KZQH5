  ...
  return (
    <Modal title={initial ? "编辑采购资料" : "新增采购资料"} onClose={onClose} size="lg">
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid gap-4 rounded-lg border border-gray-100 p-3 sm:grid-cols-2"><div><p className="mb-2 text-sm font-medium text-gray-700">资料文件</p><FileUpload purpose="catalog-draft" onUploaded={(file) => setForm((current) => ({ ...current, file_url: file.url, file_size: String(file.size), mime_type: file.mimeType }))} /><Input label="文件 URL" required value={form.file_url} onChange={(event) => update("file_url", event.target.value)} /></div><div><p className="mb-2 text-sm font-medium text-gray-700">Catalog 封面</p><FileUpload purpose="catalog-draft" label="上传封面图片" accept="image/jpeg,image/png,image/webp" hint="JPG/PNG/WebP，建议 4:3 或竖版封面。" onUploaded={(file) => update("cover_image_url", file.url)} /><Input label="封面 URL" value={form.cover_image_url} onChange={(event) => update("cover_image_url", event.target.value)} /></div></div>
