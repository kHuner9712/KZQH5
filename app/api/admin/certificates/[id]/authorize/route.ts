/**
 * Certificate authorize API (Section 6).
 *   POST /api/admin/certificates/[id]/authorize
 *
 * Marks authorization_status='confirmed' via a DEDICATED server-side
 * RPC (authorize_certificate). This is NOT a generic PATCH — the
...
import { authorizeCertificate } from "@/lib/services/admin-certificate-write";
...
export async function POST(
  ...
  const result = await authorizeCertificate(
    guard.client,
