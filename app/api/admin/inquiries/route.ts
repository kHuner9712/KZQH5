...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error("Admin inquiry list failed:", error instanceof Error ? error.message : "unknown error");
...
export async function PATCH(request: NextRequest) {
  ...
  if (!body.id) return NextResponse.json({ error: "缺少询盘 ID" }, { status: 400 });
  ...
  if (typeof body.assignee === "string") patch.assignee = body.assignee.trim().slice(0, 200) || null;

  if (isDemoMode()) {
    return NextResponse.json({ success: true, demo: true, inquiry: { id: body.id, ...patch } });
  }
  ...
  try {
    ...
    console.error("Admin inquiry update failed:", error instanceof Error ? error.message : "unknown error");
